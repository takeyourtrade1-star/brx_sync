...
class AdaptiveRateLimiter:
    ...
    def check_and_consume(self, user_id: str, tokens: int = 1) -> Tuple[bool, Optional[float]]:
        """
        ...
        try:
            ...
            result = self.redis.eval(
                lua_script, 1, key, effective_limit, self.window_seconds, now, tokens  # num keys
            )
            ...
            # External mutations/reads must not become unbounded when Redis is down.
    ...
    def record_429_response(self, user_id: str) -> None:
        ...
        try:
            ...
            self.redis.ltrim(f"{stats_key}:429_timestamps", 0, 100)
            ...
            logger.warning(
                "User %s received 429, reducing rate limit factor from %.2f to %.2f",
                user_id,
                current_factor,
                new_factor,
            )
            ...
            logger.warning(
                "Unable to record CardTrader 429 telemetry for user %s: %s",
                user_id,
                exc,
            )
    ...
    def record_success(self, user_id: str) -> None:
        ...
        try:
            ...
            recent_429s = self._get_recent_429_count(user_id, window=300)
            if recent_429s == 0:
                current_factor = self._get_adaptive_factor(user_id)
                if current_factor < 1.0:
                    new_factor = min(
                        self.max_factor,
                        current_factor * self.increase_factor,
                    )
                    self._set_adaptive_factor(user_id, new_factor)
        except Exception as exc:
            logger.warning(
                "Unable to record CardTrader success telemetry for user %s: %s",
                user_id,
                exc,
            )
    ...
    def _get_adaptive_factor(self, user_id: str) -> float:
        ...
        try:
            ...
            logger.warning(
                "Adaptive rate factor unavailable for user %s: %s",
                user_id,
                exc,
            )
            return 1.0
